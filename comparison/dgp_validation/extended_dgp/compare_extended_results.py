"""Compare R and Python CausalArima Results for Extended DGPs.

This script compares results from R's CausalArima and Python's pycausalarima
across all Extended DGPs and generates a detailed comparison report.
"""
...
# Load configuration
with open("dgp_extended_configs.json", "r") as f:
    config = json.load(f)
...
print("Comparing R vs Python Results (Extended DGPs)...")
print("=" * 50)
print()
...
for dgp in dgps:
    ...
    print(f"DGP {dgp_id}: {dgp_name} (true effect: {true_effect:+})")

    # Check if files exist
    if not os.path.exists(r_final_file):
        print(f"  - R results not found: {r_final_file}")
        report_lines.append(f"| {dgp_id} | {dgp_name} | {true_effect:+} | - | - | - | - | SKIP (no R) |")
        continue

    if not os.path.exists(py_final_file):
        print(f"  - Python results not found: {py_final_file}")
        report_lines.append(f"| {dgp_id} | {dgp_name} | {true_effect:+} | - | - | - | - | SKIP (no Py) |")
        continue
    ...
    report_lines.append(
        f"| {dgp_id} | {dgp_name} | {true_effect:+} | {point_mark} | {cum_mark} | {avg_mark} | {pval_mark} | **{status}** |"
    )
...
for res in all_results:
    ...
    report_lines.append(
        f"| {res['dgp_id']} | {res['true_effect']:+} | Point Effect | {res['r_point']:.3f} | {res['py_point']:.3f} | {diff_point:+.4f} |"
    )

    # Cumulative
    diff_cum = res["py_cum"] - res["r_cum"]
    report_lines.append(
        f"| {res['dgp_id']} | | Cumulative | {res['r_cum']:.3f} | {res['py_cum']:.3f} | {diff_cum:+.4f} |"
    )

    # Average
    diff_avg = res["py_avg"] - res["r_avg"]
    report_lines.append(
        f"| {res['dgp_id']} | | Average | {res['r_avg']:.3f} | {res['py_avg']:.3f} | {diff_avg:+.4f} |"
    )

    # P-value
    diff_pval = res["py_pval"] - res["r_pval"]
    report_lines.append(
        f"| {res['dgp_id']} | | P-value | {res['r_pval']:.4f} | {res['py_pval']:.4f} | {diff_pval:+.6f} |"
    )
